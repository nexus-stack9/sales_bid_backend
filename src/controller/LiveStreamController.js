...
const createLiveStream = async (req, res) => {
  ...
  try {
    ...
    const productCheck = await db.query(
      "SELECT product_id, name, live_input_id, is_live FROM products WHERE product_id = $1",
      [productId]
    );
    ...
    // Auto-cleanup: If product already has a live stream, delete it first
    if (product.live_input_id) {
      try {
        await cloudflareStreamService.deleteLiveInput(product.live_input_id);
        console.log(
          `Auto-cleaned existing live input: ${product.live_input_id}`
        );
      } catch (error) {
        console.warn(
          "Failed to delete existing live input (may already be deleted):",
          error.message
        );
        // Continue anyway - the old stream might already be gone
      }
    }
    ...
    const productLiveUrl = process.env.CF_LIVE_URL.replace(
      "{live_input_id}",
...
const startLiveStream = async (req, res) => {
  ...
  try {
    ...
    if (!product.rows[0].live_input_id) {
      return res
        .status(400)
        .json({ error: "No live stream configured for this product" });
    }
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        playbackUrl: cloudflareStreamService.getPlaybackUrl(
          product.live_input_id
        ),
